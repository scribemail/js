...
let finished = false;
// Closing stdin (Ctrl-D / piped EOF) mid-prompt is a clean abort, not a crash.
rl.on("close", () => {
  if (!finished) fail("Aborted (input closed).");
});
...
async function main() {
  ...
  if (branch !== "main" && !yes(await ask(`⚠ You are on "${branch}", not "main". Continue anyway? [y/N] `))) {
    fail("Aborted.");
  }
  ...
  switch (await ask("\nSelect a bump [1-4]: ")) {
    ...
    case "4": {
      ...
      if (!/^\d+\.\d+\.\d+$/.test(v)) fail(`"${v}" is not a valid x.y.z version.`);
      bumpArg = v; target = v; break;
    }
    default: fail("Aborted — no valid option selected.");
  }
